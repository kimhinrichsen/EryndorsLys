...
function calcProgress(x, max=100){ const L=calcLevel(x,max), b=xpForLevel(L), n=xpForLevel(L+1); return Math.min((x-b)/(n-b),1); }
...
function makeFreshState(){
  return {
    v:SAVE_VERSION,currentView:'main',xp:0,
    archetypeXP:Object.fromEntries(archetypes.map(a=>[a.id,0])),
    archetypeLevel:Object.fromEntries(archetypes.map(a=>[a.id,1])),
    active:[],completed:[],tavleQuests:[],chronicleLore:[],
    achievementsUnlocked:new Set(),bookPageIndex:0,minorLoreProgress:{},
    meta:{name:'Karakter',avatar:null}
  };
}
...
function achievementsForArchetype(aId){ return achievementDefs.filter(a=>a.archetype===aId); }
...
/* ---------- LORE ---------- */
function getMinorShownCount(id,l){ return state.minorLoreProgress[id]?.[l]||0; }
function setMinorShownCount(id,l,c){ if(!state.minorLoreProgress[id]) state.minorLoreProgress[id]={}; state.minorLoreProgress[id][l]=c; }
function unlockMajorLore(id,l){
  if(!LORE_CONFIG.major.archive) return;
  const lore=getArchetypeLore(id,l);
  if(!lore) return;
  const key=`${id}_${l}`;
  if(!state.chronicleLore.some(e=>e.id===key)){
    state.chronicleLore.push({id:key,archetypeId:id,archetypeName:archetypeMap[id]?.name||id,level:l,majorLore:lore.majorLore,ts:Date.now()});
  }
}
function showMajorLorePopup(id,l){ if(LORE_CONFIG.major.mode!=='popup') return;
  const lore=getArchetypeLore(id,l); if(!lore) return;
  queuePopup({title:`${archetypeMap[id]?.name||id} – Level ${l}`,archetypeId:id,variant:'lore-popup--major',bodyHtml:`<p class="lp-major">${lore.majorLore}</p>`});
}
function showMinorLorePopup(id,l,line){ if(LORE_CONFIG.minor.mode!=='popup') return;
  queuePopup({title:`${archetypeMap[id]?.name||id} – Fragment (Lvl ${l})`,archetypeId:id,variant:'lore-popup--minor',bodyHtml:`<p class="lp-minor-frag">${line}</p>`});
}
function maybeUnlockMinorLore(id){
  const xp=state.archetypeXP[id], level=calcLevel(xp), lore=getArchetypeLore(id,level);
  if(!lore||!Array.isArray(lore.minorLore)||!lore.minorLore.length) return;
  const n=lore.minorLore.length, shown=getMinorShownCount(id,level);
  if(shown>=n) return;
  const p=calcProgress(xp);
  for(let i=shown+1;i<=n;i++){
    if(p >= i/(n+1)){
      setMinorShownCount(id,level,i);
      scheduleSave('minorLoreUnlock');
      showMinorLorePopup(id,level,lore.minorLore[i-1]);
    } else break;
  }
}
function handleLevelUpLore(id,before,after){
  for(let L=before+1;L<=after;L++){
    unlockMajorLore(id,L);
    setMinorShownCount(id,L,0);
    if(state.currentView==='main') showMajorLorePopup(id,L);
  }
}
function backfillMajorLore(){
  archetypes.forEach(a=>{
    const lvl=calcLevel(state.archetypeXP[a.id]);
    for(let L=2;L<=lvl;L++){
      if(!state.chronicleLore.some(e=>e.id===`${a.id}_${L}`)) unlockMajorLore(a.id,L);
    }
  });
}
...
function basePopupHtml({title,bodyHtml,archetypeId,variant}){ const bg=getArchetypeImagePath(archetypeId); return `
  <div class="lore-popup lore-popup-appear ${variant||''} ${bg?'lore-popup--arch':''}" ${archetypeId?`data-archetype="${archetypeId}"`:''} style="${bg?`--arch-bg:url('${bg}');`:''}">
...
function buildStaticMarkup(){
  const r=root(); if(!r){ console.error('[INIT] Mangler #app root'); return; }
  r.innerHTML=`
    <div id="main-view" class="view">
      ...
      <button id="chronicle-launcher" class="chronicle-launch" title="Åbn Krøniken">
    ...
    <div id="chronicle-view" class="view" style="display:none;">
      <div class="chron-shell">
        <header class="chron-topbar">
          ...
          <div class="chron-actions"><button id="save-now" class="btn ghost">💾 Gem</button></div>
        </header>
          ...
          <div class="book">
            ...
            <div class="book-pages">
              <div class="book-pagination">
                ...
                <div class="page-indicator"><span id="book-page-current">1</span>/<span id="book-page-total">2</span></div>
...
function setBodyScrollLock(view){
  if(view==='chronicle'){ document.documentElement.classList.add('lock-scroll'); document.body.classList.add('lock-scroll'); }
  else { document.documentElement.classList.remove('lock-scroll'); document.body.classList.remove('lock-scroll'); }
}
...
function renderCharacterHeader(){
  ...
  if(av) av.innerHTML= state.meta.avatar ? `<img src='${state.meta.avatar}' alt='Avatar' class='char-avatar'>` : `<div class="char-avatar placeholder">?</div>`;
  if(nm) nm.textContent=state.meta.name||'Karakter';
}
function renderProgressBar(){
  const L=getPlayerLevel(), p=calcProgress(state.xp);
  const el=document.getElementById('progressbar'); if(!el) return;
  el.innerHTML=`<div class="kro-xp-bar-container">
...
function renderStory(){
  ...
  el.innerHTML=`<div class="kro-storybox"><h2 class="kro-storytitle">${chapter.title}</h2><div class="kro-storybody">${chapter.text}</div></div>`;
}
function renderProfile(){
  ...
  <div class="kro-mentors"><div class="kro-mentors-row">
    ${archetypes.map(a=>{
  ...
  div.querySelectorAll('.kro-mentorbox').forEach(el=>{ el.onclick=()=>showMentorOverlay(el.getAttribute('data-mentor')); });
  const edit=document.getElementById('edit-character-btn'); if(edit) edit.onclick=()=>openCharacterEdit();
}
function rarityBadge(r){
  if(!r||r==='legacy') return '';
  const map={common:{c:'#d8d0c0',bg:'#3b352c'},uncommon:{c:'#b6e8c7',bg:'#22402b'},rare:{c:'#c3d9f7',bg:'#1e2f49'},epic:{c:'#f8d28a',bg:'#4a3614'}};
...
function showMentorOverlay(id){
  ...
  const mentor=archetypes.find(a=>a.id===id); if(!mentor) return;
  let overlay=document.getElementById('mentor-overlay'); if(!overlay){ overlay=document.createElement('div'); overlay.id='mentor-overlay'; document.body.appendChild(overlay); }
  const xp=state.archetypeXP[id], lv=calcLevel(xp);
  const archAch=achievementsForArchetype(id), items=mentor.items||mentor.itemList||[];
  const achHtml=archAch.length?`<div class="mentor-subsection"><h4 class="mentor-subtitle">Achievements</h4><div class="mentor-ach-grid">${
    archAch.map(a=>`<div class="mentor-ach-item ${state.achievementsUnlocked.has(a.id)?'unlocked':''}">
      <span class="ach-name">${a.title}</span><span class="ach-status">${state.achievementsUnlocked.has(a.id)?'✓':'–'}</span><div class="ach-desc">${a.desc}</div></div>`).join('')
  }</div></div>`:`<div class="mentor-subsection"><h4 class="mentor-subtitle">Achievements</h4><em>Ingen specifikke achievements.</em></div>`;
  const itemsHtml=items.length?`<div class="mentor-subsection"><h4 class="mentor-subtitle">Items</h4><ul class="mentor-items">${
    items.map(it=> typeof it==='string'?`<li>${it}</li>`:`<li><b>${it.name||'Ukendt'}</b>${it.desc?` – <span class="item-desc">${it.desc}</span>`:''}</li>` ).join('')
  }</ul></div>`:`<div class="mentor-subsection"><h4 class="mentor-subtitle">Items</h4><em>Ingen items (endnu).</em></div>`;
  overlay.innerHTML=`
  <div class="kro-mentor-overlaybox kro-mentor-overlaybox--with-bg mentor-overlay-fixed" style="--mentor-overlay-bg:url('${getArchetypeImagePath(id)}')">
    ...
    <div class="mentor-overlay-columns">
      <div class="mentor-col mentor-col--info">
        <div class="kro-mentor-overlay-header"><span class="kro-mentor-overlay-icon">${mentor.icon||''}</span><span class="kro-mentor-overlay-title">${mentor.name}</span></div>
      ...
      <div class="mentor-col mentor-col--lists">${achHtml}${itemsHtml}</div>
    </div>
  </div>`;
  ...
  const closeBtn=document.getElementById('close-mentor-overlay'); if(closeBtn) closeBtn.onclick=closeMentorOverlay;
  overlay.addEventListener('mousedown', e=>{ if(e.target===overlay) closeMentorOverlay(); });
}
function closeMentorOverlay(){
  const ov=document.getElementById('mentor-overlay');
...
function refillTavleQuests(){
  while(state.tavleQuests.length<MAX_QUESTS_ON_TAVLE){
    const n=generateQuestList(1,{playerLevel:getPlayerLevel(),archetypeLevels:state.archetypeLevel,currentProgressCount:state.active.filter(q=>q.type==='progress').length})[0];
    if(n){ n.archetype=canonicalArchetypeId(n.archetype); state.tavleQuests.push(n); } else break;
  }
}
function renderQuests(){
  ...
  state.tavleQuests.forEach(q=>{
    const progressHtml=q.type==='progress'?`<div class="kro-quest-progress">Fremgang: ${q.progress} / ${q.vars[q.goal]} <button class="kro-btn" data-progress="${q.id}">+1</button></div>`:'';
    const el=document.createElement('div');
    el.className='kro-questroll';
    el.innerHTML=`
      <span class="kro-questicon">${q.icon||''}</span>
      <div class="quest-topline"><b>${q.name}</b>${rarityBadge(q.rarity)}</div>
...
function renderActiveQuests(){
  ...
  state.active.forEach(q=>{
    const progressHtml=q.type==='progress'?`<div class="kro-quest-progress">Fremgang: ${q.progress} / ${q.vars[q.goal]} <button class="kro-btn" data-progress-active="${q.id}">+1</button></div>`:'';
    const el=document.createElement('div');
    el.className='kro-questroll';
    el.innerHTML=`
      <span class="kro-questicon">${q.icon||''}</span>
  ...
  wrap.querySelectorAll('[data-complete]').forEach(btn=>{
    btn.onclick=()=>{
      ...
      if(idx>=0){
        ...
        const goalOk = quest.type!=='progress' || quest.completed || (quest.goal && quest.vars && quest.progress>=quest.vars[quest.goal]);
        if(!goalOk) return;
        state.completed.push({id:quest.id,name:quest.name,archetype:quest.archetype,xp:quest.xp,type:quest.type,levelRequirement:quest.level,completedAt:Date.now(),rarity:quest.rarity||''});
        state.xp+=quest.xp;
...
/* ---------- KRØNIKE ---------- */
const BOOK_PAGE_PAIR=[['stats','achievements'],['completed','lore']];
function renderBookPages(){
  const spread=document.getElementById('book-spread'); if(!spread) return;
  const total=BOOK_PAGE_PAIR.length;
  if(state.bookPageIndex>=total) state.bookPageIndex=total-1;
  ...
  spread.innerHTML=pair.map(renderBookPage).join('');
  const prev=document.getElementById('book-prev'), next=document.getElementById('book-next');
  if(prev) prev.disabled = state.bookPageIndex===0;
  if(next) next.disabled = state.bookPageIndex===total-1;
}
...
function renderStatsPage(){
  ...
  const rows=archetypes.map(a=>({name:a.name,count:state.completed.filter(q=>q.archetype===a.id).length,level:calcLevel(state.archetypeXP[a.id])}));
  return `<div class="book-page book-page--stats">
    <h3>Stats</h3>
    <div class="stats-summary"><div><b>Total Stjernelys:</b> ${state.xp}</div><div><b>Gennemførte quests:</b> ${total}</div></div>
    <div class="stats-grid">${rows.map(r=>`<div class="stat-card"><div class="stat-title">${r.name}</div><div class="stat-line"><span>Quests:</span> <b>${r.count}</b></div><div class="stat-line"><span>Level:</span> <b>${r.level}</b></div></div>`).join('')}</div>
  </div>`;
}
function renderAchievementsPage(){
  const unlocked=[],locked=[];
  achievementDefs.forEach(d=> state.achievementsUnlocked.has(d.id)?unlocked.push(d):locked.push(d));
  const block=(label,arr,ok)=>arr.length?`<div class="ach-block"><h4>${label}</h4><div class="ach-grid">${
    arr.map(a=>`<div class="ach-item ${ok?'unlocked':''}"><div class="ach-item-title">${a.title}</div><div class="ach-item-desc">${a.desc}</div><div class="ach-item-status">${ok?'✓':'Låst'}</div></div>`).join('')
  }</div></div>`:'';
  return `<div class="book-page book-page--achievements"><h3>Achievements</h3>${block('Åbnet',unlocked,true)}${block('Låste',locked,false)}</div>`;
}
function renderCompletedPage(){
  if(!state.completed.length) return `<div class="book-page book-page--completed"><h3>Gennemførte Quests</h3><em>Ingen endnu.</em></div>`;
...
document.addEventListener('click', e=>{
  if(e.target.id==='book-prev' && state.bookPageIndex>0){ state.bookPageIndex--; renderBookPages(); scheduleSave('bookPrev'); }
  if(e.target.id==='book-next' && state.bookPageIndex<BOOK_PAGE_PAIR.length-1){ state.bookPageIndex++; renderBookPages(); scheduleSave('bookNext'); }
});
...
async function fileToCompressedDataURL(file,maxSize=96){
  return new Promise((resolve,reject)=>{
    ...
    fr.onload=e=>{ img.onload=()=>{ const c=document.createElement('canvas'); const sc=Math.min(maxSize/img.width,maxSize/img.height,1); const w=Math.round(img.width*sc),h=Math.round(img.height*sc);
      c.width=w; c.height=h; c.getContext('2d').drawImage(img,0,0,w,h);
      let d; try{ d=c.toDataURL('image/jpeg',0.8);}catch(_){ d=c.toDataURL('image/png'); }
      resolve(d);
    }; img.onerror=()=>reject(new Error('Billedfejl')); img.src=e.target.result; };
...
function showProfileSelector(){
  const r=root(); if(!r){console.error('Ingen #app'); return;}
  r.innerHTML=`
  <div class="profile-select-screen">
    <h1>Eryndors Lys</h1>
    <p class="ps-intro">Vælg en eksisterende karakter eller opret en ny.</p>
    <div id="ps-error" style="display:none;background:#5c1a1a;padding:.7rem .9rem;border:1px solid #c33;color:#ffdada;border-radius:8px;margin-bottom:1rem;font-size:.8rem;"></div>
  ...
  renderProfileList();
  initAvatarChooser();
  const btn=document.getElementById('create-prof');
  const nameInput=document.getElementById('new-prof-name');
  function showErr(msg){
    const box=document.getElementById('ps-error'); if(!box) return;
  ...
  async function handleCreate(){
    ...
    try{
      const name=(nameInput.value.trim())||'Karakter';
      ...
      try{
        ...
        console.error('initAppAfterProfile fejl',e);
        showErr('Fejl under initialisering af spillet. Se console.');
        btn.disabled=false; btn.textContent='Opret Karakter';
      }
    }catch(e){
      console.error('Oprettelse fejl',e);
      showErr('Kunne ikke oprette karakter: '+e.message);
      btn.disabled=false; btn.textContent='Opret Karakter';
    }
  }
...
function renderProfileList(){
  const listEl=document.getElementById('ps-list'); if(!listEl) return;
  const profiles=listProfiles();
  if(!profiles.length){ listEl.innerHTML='<em>Ingen karakterer endnu.</em>'; return; }
  listEl.innerHTML=profiles.map(p=>`
    <div class="ps-item" data-pid="${p.id}">
      <div class="ps-left">
        ...
        <div class="ps-info"><div class="ps-name">${p.name}</div><div class="ps-meta">XP: ${p.xp}</div></div>
      </div>
  ...
  listEl.querySelectorAll('[data-del]').forEach(btn=>{
    btn.onclick=()=>{
      ...
      if(confirm('Slet denne karakter permanent?')){ deleteProfile(pid); renderProfileList(); }
    };
  });
}
...
function initAvatarChooser(){
  ...
  if(grid){
    grid.innerHTML=PRESET_AVATARS.map(a=>`<button class="avatar-choice" data-avatar="${a.src}" title="${a.label}"><img src='${a.src}' alt='${a.label}'></button>`).join('');
    grid.querySelectorAll('.avatar-choice').forEach(btn=>{
      btn.onclick=()=>{ currentAvatarSelection=btn.getAttribute('data-avatar'); updateChosenPreview(preview); };
    });
  }
  ...
  if(up){
    up.onchange=async ()=>{
      const f=up.files?.[0];
      if(f){
        try{ currentAvatarSelection=await fileToCompressedDataURL(f,96); }
        catch(e){
          console.warn('Komprimeringsfejl',e);
          const fr=new FileReader(); fr.onload=ev=> currentAvatarSelection=ev.target.result; fr.readAsDataURL(f);
        }
  ...
  if(urlInput){
    urlInput.onchange=()=>{
      ...
      if(val){ currentAvatarSelection=val; updateChosenPreview(preview); }
    };
  }
  updateChosenPreview(preview);
}
function updateChosenPreview(preview){
  if(!preview) return;
  preview.innerHTML = currentAvatarSelection
    ? `<div class="chosen-label">Valgt Avatar:</div><img src='${currentAvatarSelection}' alt='Valgt' class='chosen-avatar-img'>`
    : `<em>Ingen avatar valgt</em>`;
}
...
function openCharacterEdit(){
  ...
  wrap.innerHTML=`
    <div class="char-edit-modal">
      ...
      <label>Navn <input id="edit-char-name" value="${state.meta.name||''}" /></label>
      <div class="char-edit-avatar-block">
        ...
        <div class="char-avatar-actions">
          <div class="mini-avatar-grid">
            ${PRESET_AVATARS.map(a=>`<button class="mini-avatar-btn" data-mini-avatar="${a.src}" title="${a.label}"><img src='${a.src}' alt='${a.label}'></button>`).join('')}
          </div>
          <label class="upload-inline">Upload <input type="file" id="edit-avatar-upload" accept="image/*" hidden></label>
  ...
  if(fileInput){
    fileInput.onchange=async ()=>{
      ...
      if(f){
        try{ tempAvatar=await fileToCompressedDataURL(f,96); }
        catch(e){
          console.warn('Komprimering fejlede',e);
          const fr=new FileReader(); fr.onload=ev=>{ tempAvatar=ev.target.result; updateTempAvatar(); }; fr.readAsDataURL(f); return;
        }
        updateTempAvatar();
      }
    };
  }
  const urlInput=wrap.querySelector('#edit-avatar-url'); if(urlInput) urlInput.onchange=()=>{ const v=urlInput.value.trim(); if(v){ tempAvatar=v; updateTempAvatar(); } };
  ...
  wrap.querySelector('#char-edit-save').onclick=()=>{
    ...
    state.meta.name=newName; state.meta.avatar=tempAvatar;
    updateActiveProfileMeta({name:newName,avatar:tempAvatar});
    renderCharacterHeader(); renderProfile(); scheduleSave('charEdit'); wrap.remove();
  };
  wrap.querySelector('#char-edit-reset').onclick=()=>{
    if(confirm('Nulstil denne karakter til 0 XP og tom historik?')){
      ...
      renderAll(); scheduleSave('charSoftReset'); wrap.remove();
    }
  };
}
...
/* ---------- FULL RENDER ---------- */
function renderAll(){
  renderCharacterHeader(); renderProgressBar(); renderStory(); renderProfile(); renderQuests(); renderActiveQuests(); switchView(state.currentView); checkAchievements('renderAll');
}
...
function initAppAfterProfile(){
  ...
  if(!state.tavleQuests.length){
    state.tavleQuests = generateQuestList(MAX_QUESTS_ON_TAVLE,{playerLevel:getPlayerLevel(),archetypeLevels:state.archetypeLevel});
  }
  hookGlobalUI();
  renderAll();
...
function hookGlobalUI(){
  const chronBtn=document.getElementById('chronicle-launcher'); if(chronBtn) chronBtn.onclick=()=>switchView('chronicle');
  const backBtn=document.getElementById('back-to-main'); if(backBtn) backBtn.onclick=()=>switchView('main');
  const saveNow=document.getElementById('save-now'); if(saveNow) saveNow.onclick=()=>{ saveState('manual'); quickSaveToast(); };
  const switchBtn=document.getElementById('switch-profile'); if(switchBtn) switchBtn.onclick=()=>{ saveState(); showProfileSelector(); };
  const resetBtn=document.createElement('button'); resetBtn.textContent='Nulstil karakter'; resetBtn.className='btn danger mini char-reset-btn';
  resetBtn.onclick=()=>{ if(confirm('Nulstil denne karakter helt?')){ resetActiveProfile(makeFreshState); attachStateFromProfile(); renderAll(); scheduleSave('resetProfile'); } };
...
function init(){
  ensureProfileStore();
  const haveProfile = !!getActiveProfile();
  if(ALWAYS_SHOW_PROFILE_SELECTOR || !haveProfile) {
    showProfileSelector();
  } else {
...
window.debugArchetypeXP=function(id,amount=100){
  id=canonicalArchetypeId(id); if(!(id in state.archetypeXP)) return;
  const before=calcLevel(state.archetypeXP[id]); state.archetypeXP[id]+=amount;
  const after=calcLevel(state.archetypeXP[id]); if(after>before) handleLevelUpLore(id,before,after); else maybeUnlockMinorLore(id);
  state.archetypeLevel[id]=after; renderProfile(); scheduleSave('debugArchetypeXP');
};
window.debugAddXP=function(amount=50){ state.xp+=amount; renderProgressBar(); scheduleSave('debugAddXP'); };
